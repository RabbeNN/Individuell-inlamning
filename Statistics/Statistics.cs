...
namespace Statistics
{
    ...
    public class Statistics
    {
        ...
        public static IEnumerable<int> Mode()
        {
            //Grouping all the pairs with the same value and sorting by descending.
            var AllPairs = source.GroupBy(value => value).OrderByDescending(group => group.Count());
            int ModeCount = AllPairs.First().Count();
            //Taking all elements from AllPairs which have the same count. All those are modes of the array
            IEnumerable<int> Modes = AllPairs.Where(pair => pair.Count() == ModeCount).Select(pair => pair.Key).ToList();
            //return Modes.
            return Modes;
        }
